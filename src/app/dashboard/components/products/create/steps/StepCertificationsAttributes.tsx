 ...
 * @description Paso 7: Certificaciones y atributos - VERSIÓN COMPLETA
 * 
 * @features
 * - Gestión de certificaciones con documentos acreditativos
 * - Atributos dinámicos con ejemplos claros
 * - Tooltips inline con hover
 * - Badges con iconos y texto en la misma línea
 * - Manejo correcto de fechas
 * - Diseño 100% responsive
 * - Estilos de marca Origen
 * 
 * @version 3.5.0 - Corregido manejo de fechas
...
import { DocumentUploader, type DocumentFile } from '@/components/ui/document-uploader';
import { FieldWithHelp } from '@/components/ui/tooltip';
...
export function StepCertificationsAttributes({ 
  ...
  // Obtener ejemplos según categoría
  const categoryExamples = ATTRIBUTE_EXAMPLES[productCategory] || ATTRIBUTE_EXAMPLES.quesos;

  // Validar si el paso está completo (al menos una certificación o un atributo)
  ...
  const handleAddCertification = () => {
    const cert: Certification = {
      ...
      status: newCert.status as 'active' | 'expired' | 'pending' || 'pending',
      verified: false,
      documents: newCert.documents || [],
      verificationUrl: newCert.verificationUrl,
      category: newCert.category as any,
    };
  ...
  const handleVerifyCertification = (id: string) => {
    ...
    setTimeout(() => {
      const updated = certifications.map(cert =>
        cert.id === id ? { ...cert, verified: true, status: 'active' as const } : cert
      );
      onCertificationsChange(updated);
      ...
      <Card variant="elevated" hoverEffect="organic" className="p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isStepComplete ? (
              <IconBadge icon={<CheckCircle className="h-4 w-4" />} variant="success">Completado</IconBadge>
            ) : (
              <IconBadge icon={<AlertCircle className="h-4 w-4" />} variant="warning">Pendiente</IconBadge>
            )}
            <IconBadge icon={<Sparkles className="h-4 w-4" />} variant="leaf">Paso 7 de 8</IconBadge>
          </div>
        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Select
                            ...
                            onValueChange={(v) => setNewCert({ ...newCert, status: v as any })}
                          >
            ...
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      <span className="font-medium">Ejemplo:</span> Para un queso, puedes añadir atributos como 
                      "Tipo de leche", "Tiempo de curación" o "Textura".
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <p className="text-sm font-medium text-origen-bosque mb-3 flex items-center gap-2">
                  ...
                  Atributos comunes para {productCategory === 'quesos' ? 'quesos' : 
                                        productCategory === 'vinos' ? 'vinos' : 
                                        productCategory === 'aceites' ? 'aceites' : 'tu producto'}
                </p>
              ...
              {/* Botón añadir atributo personalizado */}
              <div className="flex justify-end">
                <button
                  onClick={() => {
                    resetAttrForm();
